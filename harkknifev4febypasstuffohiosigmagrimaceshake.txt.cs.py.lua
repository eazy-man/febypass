--[[
Grab Knife V4 (FULL)
is this even a backport -- THE REAL ROBLOX
...
if usables == true then
	...
	if game:GetService('RunService'):IsStudio() then
		...
		HeartOriginal = game:GetService("InsertService"):LoadAsset(tonumber(v))
	end
	local kidnaproom = nil
	if game:GetService('RunService'):IsStudio() then
		kidnaproom = game:GetService('InsertService'):LoadAsset('1345003780')["oof"]
	else
		kidnaproom = game:GetService("InsertService"):LoadAsset('1345003780')
	end
	...
	spawn(function()
		...
		local cur = 0
		local working = false
		kidnaproom.Touched:connect(function(hit)
...
Boom boom shoom shoom potato tomato ass tass meme team.]])
	...
	if game:GetService('RunService'):IsStudio() then
		...
		droneOriginal = game:GetService("InsertService"):LoadAsset(v)
	end
	...
	local function setR6()
		...
		if c:FindFirstChild('R6') == nil and c:FindFirstChild('Torso') == nil then
			...
			thing2.Parent = nil
			local r = game:GetService("InsertService"):LoadAsset('rbxassetid://1235403914')
			r.Name = p.Name
			r.Parent = workspace
			...
			local aaa = game:GetService("InsertService"):LoadAsset('rbxassetid://01235668916')
	...
	function start_fragmentation(position,radius,bigoof,smoloof,samecolor)
		if bigoof then
			...
			local function worldToCell(pos)
				return math.floor(pos.X / cellSize + 0.5), math.floor(pos.Y / cellSize + 0.5), math.floor(pos.Z / cellSize + 0.5)
			end
			local cx, cy, cz = worldToCell(position)
			...
			for x = -cellRadius, cellRadius do
				for y = -cellRadius, cellRadius do
					for z = -cellRadius, cellRadius do
						local dx, dy, dz = x * cellSize, y * cellSize, z * cellSize
						if (dx * dx + dy * dy + dz * dz) <= (radius * radius) then
							workspace.Terrain:SetCell(cx + x, cy + y, cz + z, 0, 0, 0)
						end
					end
				end
			end
		...
		spawn(function()
					...
					pcall(function()
						spawn(function()
							if game:GetService('RunService'):IsStudio() then
								...
								local things = game:GetService("InsertService"):LoadAsset('rbxassetid://'..v)
				...
				pcall(function()
					spawn(function()
						if game:GetService('RunService'):IsStudio() then
							...
							game:GetService("InsertService"):LoadAsset('rbxassetid://517261392').Parent = character
						end
					end)
				end)
				pcall(function()
			...
			if heded ~= false then
				...
				pcall(function()
					...
					attachment1.Rotation = Vector3.new(0,0,180)
			...
			if bans then
				...
				for i,v in pairs(bans) do
					...
					if game:GetService('RunService'):IsStudio() then
						...
						things = game:GetService("InsertService"):LoadAsset('rbxassetid://01143615944')
					end
					things.Parent = obj1
					things.Name = v
	...
	function spawned()
		...
		function dildo()
			...
			for i=1,225 do
				...
				obj1:SetPrimaryPartCFrame(CFrame.new(obj1.PrimaryPart.Position)*CFrame.Angles(math.rad(handle.Rotation.X),math.rad(handle.Rotation.Y),math.rad(handle.Rotation.Z))*CFrame.Angles(0,math.rad(180),0))
							...
							hitdetect.Touched:connect(function(hedeads)
								...
								if hedead ~= player.Character and hedead and hedead:FindFirstChildOfClass('Humanoid') and hedead:FindFirstChild('gassedlol') == nil then
									...
									pcall(function()
										...
										sound.Volume = 1
										sound:Play()
										for i,v in pairs(hedead:GetChildren()) do
											if v:IsA('BasePart') then
		...
		function katanaQ()
			...
			pcall(function()
				...
				if goteem then
					...
					for i,v in pairs(goteem:GetChildren()) do
						if v:IsA('Weld') then v:Destroy() end
					end
					goteem = nil
					if goteem2 then
						...
						for i,v in pairs(goteem2:GetChildren()) do
							if v:IsA('Weld') then v:Destroy() end
						end
						goteem2 = nil
					end
					if goteem3 then
						...
						for i,v in pairs(goteem3:GetChildren()) do
							if v:IsA('Weld') then v:Destroy() end
						end
						goteem3 = nil
					end
		...
		function kill()
			...
			pcall(function()
				...
				attachment1.Rotation = Vector3.new(90, 0, -90)
				local attachment0 = Instance.new('Attachment')
				pcall(function()
	...
	game:GetService("UserInputService").InputBegan:connect(function(inputObject, gameProcessedEvent)
		if focused == false then
			...
			if key == Enum.KeyCode.Return then
				if stage == 10 and workingonit == true then
					stage = 0
					workingonit = false
